...
def main() -> int:
    ...
    for key in [
        "origin_scan_targets",
        "architecture_scan_targets",
        "runtime_scan_targets",
        "version_readiness_checks",
    ]:
    ...
    if REGISTRY.exists():
        ...
        if registry.get("current_version") != "v0.4.0":
            findings.append(f"registry_current_version_not_v0.4.0:{registry.get('current_version')}")
        checkpoint = str(registry.get("current_checkpoint", ""))
        if "CMS-SA v0.4.0" not in checkpoint:
            findings.append("registry_checkpoint_not_v0.4.0")
        if "CMS-SA v0.4.1" not in str(registry.get("next_anchor", "")):
            findings.append("registry_next_anchor_not_v0.4.1")
    else:
        findings.append("missing_version_registry")

    passed = len(findings) == 0
    result = {
        "schema": "CMS-SA-v0.4.0-thread-rehydration-validation",
        "passed": passed,
        "version": "v0.4.0",
        "errors": len(findings),
        "findings": findings,
